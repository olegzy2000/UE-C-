...
void ULedgeDetectorComponent::BeginPlay()
{
	...
	{
		UE_LOG(LogTraversal, Warning, TEXT("ULedgeDetectorComponent::BeginPlay failed: owner is not ACharacter | Owner=%s"), *GetNameSafe(GetOwner()));
	}
}
...
bool ULedgeDetectorComponent::DetectLedge(OUT FLedgeDescription& LedgeDescription)
{
	...
	if (!IsValid(CapsuleComponent) || !IsValid(CachedCharacterOwner->GetCharacterMovement()))
	{
		return false;
	}
	...
	bool bIsDebugEnable = IsValid(DebugSubSystem) && DebugSubSystem->IsCategoryEnable(DebugCategoryLedgeDetection);
#else
